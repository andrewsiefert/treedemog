...
# FIA data
data <- readRDS("data/recruitment_data.rds")
...
# MASTIF repro data
d2 <- read_csv("data/mastif_repro_data.csv")
...
# fit the model
model <- stan_model("code/recruitment_model.stan")

fit <- sampling(model, data = data, chains = 6, cores = 6, 
                init_r = 1,
...
# save the model
saveRDS(fit, "models/recruitment_model.rds")
